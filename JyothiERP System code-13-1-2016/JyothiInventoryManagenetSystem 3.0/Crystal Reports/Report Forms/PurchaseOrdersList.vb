Public Class PurchaseOrdersList
    ...
    Sub LoadListofOrders(Optional ByVal AddSqlText As String = "")
        If IsShowPendingsOnly.Checked = True Then
            SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [Order No],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Customer Name],nettotal as [Invoice Amount],(case IsPending when 0 then 'Completed' ELSE  'Pending' END)  as [Status] from stockinvoicedetails where Isdelete=0 and IsPending=1 and vouchername='PO' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
        Else
            SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [Order No],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Customer Name],nettotal as [Invoice Amount],(case IsPending when 0 then 'Completed' ELSE  'Pending' END)  as [Status] from stockinvoicedetails where Isdelete=0 and vouchername='PO' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
        End If
    ...
    Private Sub PurchaseOrdersList_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub SalesOrderList_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        LoadDataIntoComboBox("select distinct ledgername from  stockinvoicedetails where vouchername='PO'", TxtLedgerName, "ledgername", "*All")
