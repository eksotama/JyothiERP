...
Public Class VATAllPurchaseVouchers
    ...
    Sub LoadReport()
        ...
        SQLstr = "select * from Vatclauses where VATTYPE='VAT'"
        ...
        SqlStrTotal = "SELECT SUM(nettotal) AS Tot from StockInvoiceDetails where (VoucherName='PI' or voucherName='DP')  and isdelete=0 "
        SQLstr = "select  QutoNo as [Invoice No],TransDate as [Date],sinvoiceno as[Supplier Invoice No],sinvoicedate as [Supplier Date], LedgerName as [Particulars],transtype as [Voucher Type],(select taxregno from ledgers where ledgers.ledgername=StockInvoiceDetails.ledgername) as [Sales Tax No/TIN Number] , grosstotal as [Gross]" & VatsalesSqlStr & VATSqlStr & " ,TransCode as [Trans Code] from StockInvoiceDetails where (VoucherName='PI' or voucherName='DP') and isdelete=0  "

        sumcolno = 7
        GraphSqlStr = "(VoucherName='PI' or voucherName='DP') and isdelete=0  "
        ...
        With Me.TxtList
            ...
            kd = SQLLoadGridData(SQLstr & " order by transdate ")
            kd.Rows.Add()
        ...
        Try
            ...
            For c As Integer = sumcolno To TxtList.Columns.Count - 1
        ...
        txtcollist.Items.Clear()
        For i As Integer = 0 To TxtList.Columns.Count - 1
            If TxtList.Columns(i).Name <> "Trans Code" Then
                txtcollist.Items.Add(TxtList.Columns(i).Name)

            End If
        Next
        For i As Integer = 0 To txtcollist.Items.Count - 1
            txtcollist.SetItemCheckState(i, CheckState.Checked)
        Next
        FindPayHeadTotals()
    ...
    Private Sub VATAllPurchaseVouchers_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub BtnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click, EditToolStripMenuItem.Click
        ...
        If MsgBox("Do you want to Edit ?         ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ...
            Dim K As New PurchaseControlAll(SQLGetStringFieldValue("SELECT VOUCHERNAME FROM StockInvoiceDetails WHERE TRANSCODE=" & TxtList.Item("Trans Code", TxtList.CurrentRow.Index).Value, "VOUCHERNAME"), TxtList.Item("Trans Code", TxtList.CurrentRow.Index).Value)
    ...
    Private Sub txtcollist_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtcollist.SelectedIndexChanged
        For i As Integer = 0 To txtcollist.Items.Count - 1
            If txtcollist.GetItemCheckState(i) = CheckState.Checked Then
                TxtList.Columns(txtcollist.Items(i).ToString).Visible = True
            Else
                TxtList.Columns(txtcollist.Items(i).ToString).Visible = False
            End If
    ...
    Sub FindPayHeadTotals()
        ...
        TxtList.Item("Particulars", rno).Value = "TOTALS"
