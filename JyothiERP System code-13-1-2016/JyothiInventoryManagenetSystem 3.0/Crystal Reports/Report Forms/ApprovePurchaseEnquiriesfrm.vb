Public Class ApprovePurchaseEnquiriesfrm
    ...
    Sub LoadListofOrders(Optional ByVal AddSqlText As String = "")
        ...
        Else
            If IsShowPendingsOnly.Checked = True Then
                SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [" & DBFieldName & "],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Supplier Name],nettotal as [Invoice Amount],CASE WHEN IsApproved IS NULL THEN 'WAITING' WHEN IsApproved=1 THEN 'APPROVED' ELSE 'NOTAPPROVED' END AS [Status]  from stockinvoicedetails where Isdelete=0 and vouchername=N'" & VhName & "' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ")   and (IsApproved is null or IsApproved=0) "
            Else
                SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [" & DBFieldName & "],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Supplier Name],nettotal as [Invoice Amount],CASE WHEN IsApproved IS NULL THEN 'WAITING' WHEN IsApproved=1 THEN 'APPROVED' ELSE 'NOTAPPROVED' END AS [Status]  from stockinvoicedetails where Isdelete=0 and vouchername=N'" & VhName & "' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ")   "
            End If
        End If
    ...
    Private Sub BtnApprove_Click(sender As System.Object, e As System.EventArgs) Handles BtnApprove.Click
        ...
        If TxtList.Item("Status", TxtList.CurrentRow.Index).Value = "APPROVED" Then
            If MsgBox("This Purchase Enquiry is already Approved...", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then
                ExecuteSQLQuery("update stockinvoicedetails set IsApproved=0 where transcode=" & TxtList.Item("transcode", TxtList.CurrentRow.Index).Value)
                LoadListofOrders()
            End If
            Exit Sub
        End If
        ...
        If MsgBox("Do you want to Approve ?              ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ExecuteSQLQuery("update stockinvoicedetails set IsApproved=1 where transcode=" & TxtList.Item("transcode", TxtList.CurrentRow.Index).Value)
            LoadListofOrders()
        End If
    End Sub
End Class