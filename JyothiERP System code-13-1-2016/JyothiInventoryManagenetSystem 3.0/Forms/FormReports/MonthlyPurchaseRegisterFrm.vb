...
Public Class MonthlyPurchaseRegisterFrm
    ...
    Sub LoadReport()
        ...
        End If
        SqlStrTotal = "SELECT SUM(nettotal) AS Tot from StockInvoiceDetails where (VoucherName IN ('PI','DP'))  and isdelete=0 "
        SqlStr = "SELECT (UPPER(left(DATENAME(MONTH,StockInvoiceDetails.TransDate),3)) + '-' + CONVERT(varchar(30),year(StockInvoiceDetails.TransDate))) AS [Period], SUM(nettotal) AS [Net Total] from StockInvoiceDetails where  (VoucherName IN ('PI','DP'))   and isdelete=0 "
        GraphSqlStr = "  (VoucherName IN ('PI','DP'))  and isdelete=0  "
    ...
    Private Sub MonthlyPurchaseRegisterFrm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub TxtLedgerName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtLedgerName.SelectedIndexChanged, TxtStockGroup.SelectedIndexChanged, TxtSalesAc.SelectedIndexChanged
        LoadReport()
    End Sub
    ...
    Private Sub TxtList_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles TxtList.CellDoubleClick
        ...
        PurchaseRegister.TxtStartDate.Value = stdt.Value
        PurchaseRegister.TxtEndDate.Value = eddt.Value
