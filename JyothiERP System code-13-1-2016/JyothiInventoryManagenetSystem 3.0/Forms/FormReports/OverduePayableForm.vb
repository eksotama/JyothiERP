...
Public Class OverduePayableForm
    ...
    Sub LoadData()
        ...
        SQLstr = "select transdate as [Trans Date],refno as [Ref No],InvoiceNo AS [Invoice No], transcode as [Trans Code],ledgername as [Supplier Name], dr+cr as [Bill Amount], " _
        ...
        If TxtLedgerName.Text.Length = 0 Or TxtLedgerName.Text = "*All" Then
            SQLstr = SQLstr & " and (ledgername in (select ledgername from ledgers where  accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.SuppliersAccounts & "'))) "
        Else
        ...
        Try
            ...
            TxtList.Columns("Supplier Name").AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill
            TxtList.Columns("Supplier Name").Width = 500
    ...
    Private Sub OverduePayableForm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub TxtLedgerName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtLedgerName.SelectedIndexChanged
        ...
        Else
            TxtHeading.Text = " OVERDUE PAYABLES FOR A/C: " & UCase(TxtLedgerName.Text)
