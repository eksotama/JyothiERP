...
#Region "Functions"
    Sub loadstockgroups(Optional ByVal SearchStr As String = "")
        ...
        Sqlstr = "SELECT ROW_NUMBER() OVER (ORDER BY StockgroupName) AS [S.NO],rtrim(StockgroupName) AS [STOCK GROUP],rtrim(groupRoot) AS [UNDER GROUP] FROM stockgroups"
    ...
    Sub LoadDatainListView(Optional ByVal SearchSqlstr As String = "")
        ...
        If SearchSqlstr.Length = 0 Then
            SqlStr = "select StockgroupName from stockgroups WHERE grouplevel=2 "
        Else
            SqlStr = "select StockgroupName from stockgroups " & SearchSqlstr & " and grouplevel=2"
        End If
    ...
    Sub LoadSubNodes(ByVal GpName As String, ByVal nodeToAddTo As TreeNode)
        ...
        Try
            ...
            cnn.Open()
            Dim dscmd As New SqlDataAdapter("select StockgroupName from stockgroups where grouproot=N'" & GpName & "'", cnn)
...
#End Region
    Private Sub BtnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNew.Click, NewToolStripMenuItem.Click
        Dim K As New CreateStockGroups
        K.ShowDialog()
        loadstockgroups()
    End Sub

    Private Sub StockGroups_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub BtnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, DeleteToolStripMenuItem.Click
        ...
        Else
            If TxtList.SelectedRows.Count = 0 Then Exit Sub
            If TxtList.Item("STOCK GROUP", TxtList.CurrentRow.Index).Value = "*Primary" Then
                MsgBox("The Primary group is not editable....", MsgBoxStyle.Information)
                Exit Sub
            End If
            StockGrpName = TxtList.Item("STOCK GROUP", TxtList.CurrentRow.Index).Value
        End If



        If SQLIsFieldExists("select stockgroup from stockdbf where stockgroup=N'" & StockGrpName & "'") = True Then
            MsgBox("The Stock group already used in stock items, it is not possible to delete ...", MsgBoxStyle.Critical)
            Exit Sub
        End If
        If MsgBox("Do you want to delete ?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ExecuteSQLQuery("delete from stockgroups where StockgroupName=N'" & StockGrpName & "'")
            loadstockgroups()
        End If
    End Sub
    ...
    Private Sub BtnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click, EditToolStripMenuItem.Click
        ...
        If MsgBox("Do you want to Edit ?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            Dim K As New CreateStockGroups(StockGrpName)
            K.ShowDialog()
            loadstockgroups()
        End If
    End Sub
    ...
    Private Sub ImsButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImsButton4.Click, PrintToolStripMenuItem.Click
        ...
        cnn = New SqlConnection(ConnectionStrinG)
        cnn.Open()
        Dim dscmd As New SqlDataAdapter("SELECT * from Stockgroups", cnn)
        dscmd.Fill(ds, "Stockgroups")
        cnn.Close()
        Dim objRpt As New StockGroupMasterReport
        ...
        If PrintOptionsforCR.IsPrintHeader = False Then
            ...
            CType(objRpt.Section1.ReportObjects.Item("txttitle"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "STOCK GROUPS REPORTS"
        ...
        Else
            CType(objRpt.Section1.ReportObjects.Item("TxtPeriod"), CrystalDecisions.CrystalReports.Engine.TextObject).ObjectFormat.EnableCanGrow = True
            CType(objRpt.Section1.ReportObjects.Item("TxtPeriod"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "STOCK GROUPS REPORTS"
    ...
    Private Sub BtnImportBrowse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnImportBrowse.Click
        Dim K As New StockGroupImportExport
        K.ShowDialog()
        K.Dispose()
        loadstockgroups()
    End Sub
