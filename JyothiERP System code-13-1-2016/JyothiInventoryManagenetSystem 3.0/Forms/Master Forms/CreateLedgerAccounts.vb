...
Public Class CreateLedgerAccounts
    ...
    Dim IsPayrollLedger As Boolean = False
    ...
    Sub loadDefaults()
        ...
        If DefDepartment() = "Admin & Payroll" Then
            LoadDataIntoComboBox("select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.IndirectExpenses & "'", TxtUnderGroup, "subgroup")
        Else
            LoadDataIntoComboBox("select GroupName from AccountGroups ", TxtUnderGroup, "GroupName")
        End If
    ...
    Sub LoadonOpen()
        ...
        Try
            ...
            While Sreader.Read()
                ...
                TxtAccountNo.Text = Sreader("BankAccNo").ToString
                If IsDBNull(Sreader("n1")) = True Then
                    IsPayrollLedger = False
                Else
                    If Sreader("n1") = 2 Then
                        IsPayrollLedger = True
                    Else
                        IsPayrollLedger = False
                    End If
                End If
    ...
    Private Sub CreateSuppliers_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsOpenForAlter = True Then
            ...
            loadDefaults()
            IsOpenForAlter = True
            LoadDataIntoComboBox("select Ledgername from ledgers ", TxtSeletedLedgerName, "ledgername")
    ...
    End Sub
    Sub CreateFixedAsset()
        If SQLIsFieldExists("SELECT TOP 1 1 from   ledgers where ledgername=N'" & TxtLedgerName.Text & "' and   (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.FixedAssets & "'))") = True Then
            Dim SqlStr As String = ""
            SqlStr = " INSERT INTO [assets]([AssetID],[AssetName],[AssetBarcode],[createddate],[purchasedate],[duedate],[LastServiceDate],[LastCheckoutDate],[LastCheckInDate],[assettype],[vender],[status],[transcode],[qty],[rate],[totalvalue],[condition],[orderno],[locationid],[employeename],[department],[notes],[notesId],[expirydate],[deprate],[username],[location],[depreciationmethod],[assetimage],[IsReadOnly])     VALUES " _
                & " (@AssetID,@AssetName,@AssetBarcode,@createddate,@purchasedate,@duedate,@LastServiceDate,@LastCheckoutDate,@LastCheckInDate,@assettype,@vender,@status,@transcode,@qty,@rate,@totalvalue,@condition,@orderno,@locationid,@employeename,@department,@notes,@notesId,@expirydate,@deprate,@username,@location,@depreciationmethod,@assetimage,@IsReadOnly)    "
