...
#Region "Functions"
    ...
    Sub LoadEmployeeList(Optional ByVal sqlText As String = "")
        ...
        If TxtShowAll.Checked = True Then
            Sqlstr = "SELECT ROW_NUMBER() OVER (ORDER BY DocName) AS [S.NO],DocType as [Doc Type],DocRefNo [Doc Ref],DocName as [Document Name],DocID as [Doc ID],IssuedBy as [Issued By],ExpiryDate as [Expiry],ContactPerson as [Contact Person], PersonAddress AS [Address] ,(case when ISACTIVE=0 then 'NO' else 'YES' end) as [Active] from Documents where isdelete>=0"
        ...
        Else
            ...
            RepSqlStr = RepSqlStr & " and doctype=N'" & TxtDocType.Text & "'"
        End If
        Try
...
#End Region
    ...
    Private Sub TxtList_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles TxtList.RowEnter
        ...
        v_isactive = SQLGetNumericFieldValue("SELECT ISACTIVE FROM documents WHERE docname=N'" & TxtList.Item("Document Name", e.RowIndex).Value & "'", "ISACTIVE")
    ...
    Private Sub btnActivate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnActivate.Click
        ...
        If BtnActivate.Text = "Activate" Then
            If MsgBox("Do you want to Activate the selected Docs Account..", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then
                ExecuteSQLQuery("UPDATE documents SET ISACTIVE=1 WHERE docname=N'" & TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value & "'")
                BtnActivate.Text = "DeActivate"
                LoadEmployeeList()
                TxtList.Focus()
            End If
        Else
            If MsgBox("Do you want to De-Activate the selected Docs Account..", MsgBoxStyle.YesNo + MsgBoxStyle.Exclamation) = MsgBoxResult.Yes Then
                ExecuteSQLQuery("UPDATE documents SET ISACTIVE=0 WHERE docname=N'" & TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value & "'")
                BtnActivate.Text = "Activate"
                LoadEmployeeList()
                TxtList.Focus()
            End If
    ...
    Private Sub BtnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click, EditToolStripMenuItem.Click
        ...
        End If
        Dim v_isactive As Integer
        v_isactive = SQLGetNumericFieldValue("SELECT ISACTIVE FROM documents WHERE docname=N'" & TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value & "'", "ISACTIVE")
        If v_isactive = 0 Then
            MsgBox("This Docs Account is In-Active...., Edit is not possible....")
            Exit Sub
        End If
        Dim k As New CreateNewDocs(TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value)
        k.ShowDialog()
        k.Dispose()
        LoadEmployeeList()
    End Sub
    ...
    Private Sub BtnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click, DeleteToolStripMenuItem.Click
        If TxtList.SelectedRows.Count = 0 Then
            MsgBox("Please Select the Document Name to delete...", MsgBoxStyle.Information)
            Exit Sub
        End If
        If MsgBox("Do you want to Deleted the selected Docs Account..", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then
            ExecuteSQLQuery("UPDATE documents SET isdelete=1 WHERE docname=N'" & TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value & "'")
            LoadEmployeeList()
            TxtList.Focus()
        End If
    ...
    Private Sub TxtLedgerName_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtDocName.TextChanged
        If TxtDocName.Text.Length = 0 Then
            LoadEmployeeList()
        Else
            LoadEmployeeList("  and  DocName LIKE N'%" & TxtDocName.Text & "%'")
        End If
    End Sub

    Private Sub CompanyDocs_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub LinkLabel1_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LinkLabel1.LinkClicked
        ...
        Dim kfrm As New ViewDocFilesfrm(TxtList.Item("Document Name", TxtList.CurrentRow.Index).Value)
        kfrm.ShowDialog()
        kfrm.Dispose()
    End Sub
    ...
    Private Sub ImsButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImsButton4.Click, PrintToolStripMenuItem.Click
        ...
        Next
        Dim objRpt As New DocDetailsCrReport
        ...
        If PrintOptionsforCR.IsPrintHeader = False Then
            ...
            CType(objRpt.Section1.ReportObjects.Item("txttitle"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "COMPANY DOCUMENTS LIST"
        Else
            CType(objRpt.Section1.ReportObjects.Item("TxtPeriod"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "COMPANY DOCUMENTS LIST"
    ...
    Private Sub LinkLabel2_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LinkLabel2.LinkClicked
        ...
        If TxtList.RowCount = 0 Then Exit Sub
        If TxtList.CurrentRow.Index < 0 Then Exit Sub
        Dim kfrm As New DocViewFullFrm
        kfrm.ShowDialog()
        kfrm.Dispose()
    End Sub

    Private Sub TxtDocType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtDocType.SelectedIndexChanged
        LoadEmployeeList()
    End Sub
End Class