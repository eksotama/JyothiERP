...
Public Class OutgoiningStock
    ...
    Sub LoadData()
        ...
        Dim TotSqlStr As String = " FROM StockInvoiceRowItems WHERE (VoucherName = 'SD' or VoucherName = 'POS') and isdelete=0 "
        SqlStr = "SELECT StockCode as [Stock Code], StockSize as [More Info], SUM(TotalQty) AS [Base Qty], (CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor(SUM(TotalQty) / unitcon)) + ' ' + mainunit ELSE (CASE CAST(SUM(TotalQty) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor(SUM(TotalQty) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor(SUM(TotalQty) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST(SUM(TotalQty) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty], SUM(StockAmount) AS [Stock Value] FROM StockInvoiceRowItems WHERE (VoucherName = 'SD' or VoucherName = 'POS') and isdelete=0 "
        ...
        Try
            ...
            TxtList.Columns("Base Qty").AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            TxtList.Columns("Base Qty").Width = 110
            TxtList.Columns("Base Qty").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight



            TxtList.Columns("Total Qty").AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            TxtList.Columns("Total Qty").Width = 160
            TxtList.Columns("Total Qty").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            TxtList.Columns("Stock Value").AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            TxtList.Columns("Stock Value").Width = 110
            TxtList.Columns("Stock Value").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
    ...
    Private Sub OutgoiningStock_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
