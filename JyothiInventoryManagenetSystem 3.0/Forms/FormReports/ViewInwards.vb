...
Public Class ViewInwards

    Dim sQLSTR As String = ""
    Dim IsonLoading As Boolean = True
    Dim OpenedStockCode As String = ""
    Dim IsInwardReport As Boolean = True
    Sub New(ByVal isInward As Boolean, Optional ByVal StockCode As String = "")

        ' This call is required by the designer.
        InitializeComponent()
        If StockCode.Length > 0 Then
            OpenedStockCode = StockCode
            TxtStockCode.Items.Add(StockCode)
        End If
        IsInwardReport = isInward
    ...
    Private Sub ViewInwards_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub StockItemwiseReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsInwardReport = True Then
            'LoadData(" and stockcode=N'" & TxtStockCode.Text & "'")
            TxtShowSuppliers.Checked = True
        Else
            TxtShowSuppliers.Checked = False
        End If
    End Sub
    Sub LoadData(Optional ByVal SubStrSql As String = "")
        ...
        & "(CASE WHEN vouchername = 'SD' OR  vouchername = 'POS' THEN stockamount WHEN vouchername='SR' then -1*stockamount ELSE 0 END) AS [Outwards Value] from StockInvoiceRowItems where isdelete=0  "
            ...
            If TxtLedgerName.Text.Length = 0 Or TxtLedgerName.Text = "*All" Then
                sQLSTR = sQLSTR & SubStrSql & " and (vouchername='PG' OR vouchername='SR'  OR vouchername = 'DP') "

                If TxtShowSuppliers.Checked = True Then
                    sQLSTR = sQLSTR & " and (transcode in (select transcode from StockInvoiceDetails where ledgername in (select ledgername from ledgers where  accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.SuppliersAccounts & "'  or groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "'))))"
                Else
                    sQLSTR = sQLSTR & "  and (transcode in (select transcode from StockInvoiceDetails where ledgername in (select ledgername from ledgers where  accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CustomersAccounts & "'  or groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "'))))"
                End If
                SqlTot = SqlTot & SubStrSql
            Else

                sQLSTR = sQLSTR & SubStrSql & " and (vouchername='PG' OR vouchername='SR'  OR vouchername = 'DP' ) and (transcode in (select transcode from StockInvoiceDetails where ledgername=N'" & TxtLedgerName.Text & "'))"
                SqlTot = SqlTot & SubStrSql
            End If
        ...
        Else
           ...
           & "(CASE WHEN vouchername = 'SD' OR  vouchername = 'POS' THEN stockamount WHEN vouchername='SR' then -1*stockamount ELSE 0 END) AS [Outwards Value] from StockInvoiceRowItems where isdelete=0  "
            ...
            If TxtLedgerName.Text.Length = 0 Or TxtLedgerName.Text = "*All" Then
                sQLSTR = sQLSTR & SubStrSql & " and (vouchername='SD' or vouchername='PR' OR vouchername = 'POS')"
            ...
            Else
                ...
                sQLSTR = sQLSTR & SubStrSql & " and (vouchername='SD' or vouchername='PR' OR vouchername = 'POS') and (transcode in (select transcode from StockInvoiceDetails where ledgername=N'" & TxtLedgerName.Text & "'))"
