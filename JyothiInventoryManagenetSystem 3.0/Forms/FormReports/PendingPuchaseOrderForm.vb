...
Public Class PendingPuchaseOrderForm
    ...
    Private Sub TxtFilterByLedger_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtFilterByLedger.SelectedIndexChanged
        If TxtFilterByLedger.Text.Length = 0 Or TxtFilterByLedger.Text = "*All" Then
            ...
            SqlStr = "select stockcode as [Item Code],stockname as [Item Name],stocksize as[More Info],barcode as [Barcode],(CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainunit ELSE (CASE CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty],Transdate as [Date], qutoref as [Ref No],(select ledgername from StockInvoiceDetails where transcode=StockInvoiceRowItems.transcode) as [Customer Name] from StockInvoiceRowItems where isdelete<>1 and  vouchername='PO'   "
            LoadData()
        Else
            SqlStr = "select stockcode as [Item Code],stockname as [Item Name],stocksize as[More Info],barcode as [Barcode],(CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainunit ELSE (CASE CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty],Transdate as [Date], qutoref as [Ref No],(select ledgername from StockInvoiceDetails where transcode=StockInvoiceRowItems.transcode ) as [Customer Name] from StockInvoiceRowItems where isdelete<>1 and  vouchername='PO'  and transcode in (select transcode from StockInvoiceDetails where ledgername=N'" & TxtFilterByLedger.Text & "')   "
            LoadData()
        End If
    End Sub

    Private Sub PendingPuchaseOrderForm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub TxtFilterByItemName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtFilterByItemName.SelectedIndexChanged
        If TxtFilterByItemName.Text.Length = 0 Or TxtFilterByItemName.Text = "*All" Then
            ...
            LoadData()
        Else
            SqlStr = "select stockcode as [Item Code],stockname as [Item Name],stocksize as[More Info],barcode as [Barcode],(CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainunit ELSE (CASE CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty],Transdate as [Date], qutoref as [Ref No],(select ledgername from StockInvoiceDetails where transcode=StockInvoiceRowItems.transcode and ledgername=N'" & TxtFilterByLedger.Text & "') as [Customer Name] from StockInvoiceRowItems where isdelete<>1 and  vouchername='PO'  and stockname=N'" & TxtFilterByItemName.Text & "' "
            LoadData()
        End If
    End Sub
    ...
    Private Sub TxtFilterbyRef_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtFilterbyRef.SelectedIndexChanged
        If TxtFilterbyRef.Text.Length = 0 Or TxtFilterbyRef.Text = "*All" Then
            ...
            LoadData()
        Else
            SqlStr = "select stockcode as [Item Code],stockname as [Item Name],stocksize as[More Info],barcode as [Barcode],(CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainunit ELSE (CASE CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty],Transdate as [Date], qutoref as [Ref No],(select ledgername from StockInvoiceDetails where transcode=StockInvoiceRowItems.transcode and ledgername=N'" & TxtFilterByLedger.Text & "') as [Customer Name] from StockInvoiceRowItems where isdelete<>1 and  vouchername='PO'  and qutoref=N'" & TxtFilterbyRef.Text & "' "
            LoadData()
        End If
    End Sub
    ...
    Private Sub TxtSearchByItemName_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtSearchByItemName.TextChanged
        If TxtSearchByItemName.Text.Length = 0 Then
            ...
            LoadData()
        Else
            SqlStr = "select stockcode as [Item Code],stockname as [Item Name],stocksize as[More Info],barcode as [Barcode],(CASE (SELECT unittype FROM stockunits WHERE stockunits.unitname = StockInvoiceRowItems.baseunit) WHEN 0 THEN CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainunit ELSE (CASE CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT) WHEN 0 THEN (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit) ELSE (CONVERT(varchar(30), floor((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) / unitcon)) + ' ' + mainUnit + ' ' + CONVERT(varchar(30), CAST((TotalQty - (CASE WHEN UsedQty > 0 THEN usedqty ELSE 0 END)) AS INT) % CAST(UnitCon AS INT)) + ' ' + subUnit) END) END) AS [Total Qty],Transdate as [Date], qutoref as [Ref No],(select ledgername from StockInvoiceDetails where transcode=StockInvoiceRowItems.transcode) as [Customer Name] from StockInvoiceRowItems where isdelete<>1 and  vouchername='PO'  and stockname LIKE N'%" & TxtSearchByItemName.Text & "%' "
            LoadData()
        End If
    End Sub
