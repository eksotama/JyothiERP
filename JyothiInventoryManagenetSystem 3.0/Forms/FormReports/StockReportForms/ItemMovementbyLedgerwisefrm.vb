...
Public Class ItemMovementbyLedgerwisefrm
    Dim SQlstr As String = ""
    Dim LoadType As Integer = 1
    Dim ReportTitle As String = ""

    Private Sub ItemMovementbyLedgerwisefrm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Sub LoadData()
        ...
        If IsDateWiseOn.Checked = True Then
            ...
            Else
                ...
                WHERESQL = " WHERE  (STOCKNAME IN (SELECT STOCKNAME FROM STOCKDBF WHERE STOCKGROUP=N'" & TxtStockGroup.Text & "')) AND  (VoucherName IN ('POS', 'SD'))  and isdelete=0    and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
    ...
    Sub editinvoice()
        If TxtList.SelectedRows.Count = 1 Then
            ...
            MainForm.Cursor = Cursors.WaitCursor
            Dim K As New SalesControlAll(SQLGetStringFieldValue("SELECT VOUCHERNAME FROM StockInvoiceDetails WHERE TRANSCODE=" & TxtList.Item("TransCode", TxtList.CurrentRow.Index).Value, "VOUCHERNAME"), TxtList.Item("TransCode", TxtList.CurrentRow.Index).Value, SQLGetStringFieldValue("SELECT Transtype FROM StockInvoiceDetails WHERE TRANSCODE=" & TxtList.Item("TransCode", TxtList.CurrentRow.Index).Value, "Transtype"))
