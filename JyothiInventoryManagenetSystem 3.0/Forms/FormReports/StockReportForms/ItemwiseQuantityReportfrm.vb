...
Public Class ItemwiseQuantityReportfrm
    Dim SQlstr As String = ""
    Dim LoadType As Integer = 1
    Dim ReportTitle As String = ""

    Private Sub ItemwiseQuantityReportfrm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
        ...
        If IsDateWiseOn.Checked = True Then
            If TxtStockGroup.Text.Length = 0 Or TxtStockGroup.Text = "*All" Then
                If TxtStockLocation.Text.Length = 0 Or TxtStockLocation.Text = "*All" Then
                    ...
                    WHERESQL = " WHERE (VoucherName IN ('POS', 'SD'))  and isdelete=0    and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
                Else
                    ReportTitle = "For Location:" & TxtStockLocation.Text & "  Period " & TxtStartDate.Value.Date & " - " & TxtEndDate.Value.Date
                    WHERESQL = " WHERE LOCATION=N'" & TxtStockLocation.Text & "' AND (VoucherName IN ('POS', 'SD'))  and isdelete=0    and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
                End If
            ...
            Else
                ...
                Else
                    ...
                    WHERESQL = " WHERE (STOCKNAME IN (SELECT STOCKNAME FROM STOCKDBF WHERE STOCKGROUP=N'" & TxtStockGroup.Text & "')) AND LOCATION=N'" & TxtStockLocation.Text & "' AND (VoucherName IN ('POS', 'SD'))  and isdelete=0    and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") "
        ...
        Else
            If TxtStockGroup.Text.Length = 0 Or TxtStockGroup.Text = "*All" Then
                If TxtStockLocation.Text.Length = 0 Or TxtStockLocation.Text = "*All" Then
                    ...
                    WHERESQL = " WHERE (VoucherName IN ('POS', 'SD'))  and isdelete=0   "
                Else
                    ReportTitle = "For Location:" & TxtStockLocation.Text & "  For All Stock Groups"
                    WHERESQL = " WHERE LOCATION=N'" & TxtStockLocation.Text & "' AND  (VoucherName IN ('POS', 'SD'))  and isdelete=0 "
                End If

            Else
                If TxtStockLocation.Text.Length = 0 Or TxtStockLocation.Text = "*All" Then
                    ReportTitle = "For Stock Group : " & TxtStockGroup.Text
                    WHERESQL = " WHERE (STOCKNAME IN (SELECT STOCKNAME FROM STOCKDBF WHERE STOCKGROUP=N'" & TxtStockGroup.Text & "')) AND (VoucherName IN ('POS', 'SD'))  and isdelete=0   "
                Else
                    ReportTitle = "For Location:" & TxtStockLocation.Text & "  For Stock Group : " & TxtStockGroup.Text
                    WHERESQL = " WHERE (STOCKNAME IN (SELECT STOCKNAME FROM STOCKDBF WHERE STOCKGROUP=N'" & TxtStockGroup.Text & "')) AND LOCATION=N'" & TxtStockLocation.Text & "' AND  (VoucherName IN ('POS', 'SD'))  and isdelete=0   "
                End If
