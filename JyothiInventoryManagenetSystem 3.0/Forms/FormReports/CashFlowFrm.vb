...
Public Class CashFlowFrm
    ...
    Sub LoadDetailedReport()
        ...
        ledstr = "(ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
        If TxtShowDrCrStyle.Checked = True Then
            SqlStr = "SELECT (UPPER(left(DATENAME(MONTH,ledgerbook.TransDate),3)) + '-' + CONVERT(varchar(30),year(ledgerbook.TransDate))) AS [Period],SUM(DR) AS [Inflow],SUM(CR) as [OutFlow],(CASE  WHEN SUM(DR)-SUM(CR)>=0 THEN CONVERT(varchar(30), CAST(SUM(DR)-SUM(CR) AS DECIMAL(15,3)),1) +' Dr' ELSE CONVERT(varchar(30), CAST(SUM(CR)-SUM(DR) AS DECIMAL(15,3)),1) + ' Cr' END)  AS [Net Flow] from ledgerbook where isdeleted=0 and (ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
            SqlStr = SqlStr & " GROUP BY year(ledgerbook.TransDate),month(ledgerbook.TransDate) ,DATENAME(MONTH,TransDate) "
        Else
            SqlStr = "SELECT (UPPER(left(DATENAME(MONTH,ledgerbook.TransDate),3)) + '-' + CONVERT(varchar(30),year(ledgerbook.TransDate))) AS [Period],SUM(DR) AS [Inflow],SUM(CR) as [OutFlow], (SUM(DR)-SUM(CR))  AS [Net Flow] from ledgerbook where isdeleted=0 and (ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
            SqlStr = SqlStr & " GROUP BY year(ledgerbook.TransDate),month(ledgerbook.TransDate) ,DATENAME(MONTH,TransDate) "
        ...
        If CDbl(txtDrTotal.Text) - CDbl(TxtCrTotal.Text) > 0 Then
            If TxtShowDrCrStyle.Checked = True Then
                TxtNetTotal.Text = CompDetails.Currency & " Dr " & FormatNumber(CDbl(txtDrTotal.Text) - CDbl(TxtCrTotal.Text), ErpDecimalPlaces).ToString & " Dr"
            Else
    ...
    Private Sub CashFlowFrm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub BtnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrint.Click, PrintToolStripMenuItem.Click
        ...
        ledstr = "(ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
        If TxtShowDrCrStyle.Checked = True Then
            sqltemp = "SELECT (UPPER(left(DATENAME(MONTH,ledgerbook.TransDate),3)) + '-' + CONVERT(varchar(30),year(ledgerbook.TransDate))) AS [Period],SUM(DR) AS [DrAmount],SUM(CR) as [CrAmount],(CASE  WHEN SUM(DR)-SUM(CR)>=0 THEN CONVERT(varchar(30), CAST(SUM(DR)-SUM(CR) AS DECIMAL(15,3)),1) +' Dr' ELSE CONVERT(varchar(30), CAST(SUM(CR)-SUM(DR) AS DECIMAL(15,3)),1) + ' Cr' END)  AS [Balance] from ledgerbook where isdeleted=0 and (ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
            sqltemp = sqltemp & " GROUP BY year(ledgerbook.TransDate),month(ledgerbook.TransDate) ,DATENAME(MONTH,TransDate) "
        Else
            sqltemp = "SELECT (UPPER(left(DATENAME(MONTH,ledgerbook.TransDate),3)) + '-' + CONVERT(varchar(30),year(ledgerbook.TransDate))) AS [Period],SUM(DR) AS [DrAmount],SUM(CR) as [CrAmount], (SUM(DR)-SUM(CR))  AS [Balance] from ledgerbook where isdeleted=0 and (ledgername in (SELECT LEDGERNAME FROM LEDGERS where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CashAccounts & "' or groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' ))))"
            sqltemp = sqltemp & " GROUP BY year(ledgerbook.TransDate),month(ledgerbook.TransDate) ,DATENAME(MONTH,TransDate) "
        ...
        cnn.Open()
        Dim dscmd As New SqlDataAdapter(sqltemp, cnn)
