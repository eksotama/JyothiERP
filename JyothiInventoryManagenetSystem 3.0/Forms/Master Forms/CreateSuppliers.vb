...
Public Class CreateSuppliers
    ...
    Dim IsOpenToAlter As Boolean = False
    Dim OpenedTransCode As Single = 0
    Dim OpendeCode As String = ""
    Dim SupplierNameToEdit As String = ""
    Dim EditValues As New EditValuesStruct
    Dim OpenedCurrencyRate As Double = 1
    Dim isImagechanged As Double = False
    Sub loadDefaults()
        TabControl1.SelectedTab = TabControl1.TabPages(0)
        TxtCode.Text = GetIDCode(EnumIDType.SupplierID)
        TxtLedgerName.Text = ""
        TxtAcitvity.Text = ""
        ...
        LoadDataIntoComboBox("select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.SuppliersAccounts & "'", TxtUnderGroup, "subgroup")
        ...
        IsOpenToAlter = False
    End Sub
    Sub New(Optional ByVal EditsupplierName As String = "")

        ' This call is required by the designer.
        InitializeComponent()

        If EditsupplierName.Length > 0 Then
            SupplierNameToEdit = EditsupplierName
            IsOpenToAlter = True
        End If
    ...
    Sub LoadonOpen()
        ...
        Try
            ...
            Sqlcmmd.CommandText = "select * from ledgers where ledgername=N'" & SupplierNameToEdit & "'"
        ...
        If EditValues.isBillWise = 1 Then
            ...
            Try
                ...
                Sqlcmmd1.CommandText = "select * from Bill2bill where ledgername=N'" & SupplierNameToEdit & "' and isopening=1"
        ...
        End If
        Try
            TxtOpDate.Value = SQLGetStringFieldValue("select transdate from ledgerbook where ledgername=N'" & SupplierNameToEdit & "'", "transdate")
        Catch ex As Exception

        End Try
        If SQLIsFieldExists("SELECT TOP 1 1 from   ledgerbook where ledgername=N'" & SupplierNameToEdit & "'") = True Then
    ...
    Private Sub CreateSuppliers_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub CreateSuppliers_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsOpenToAlter = True Then
            loadDefaults()
            BtnSave.Text = "&Alter"
            IsOpenToAlter = True
            'TxtSeletedLedgerName
            LoadDataIntoComboBox("select Ledgername from ledgers where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.SuppliersAccounts & "')) ", TxtSeletedLedgerName, "ledgername")
            TxtSeletedLedgerName.Text = SupplierNameToEdit

        Else
            editPanel2.Enabled = False
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        End If
        If IsOpenToAlter = True Then
            If UCase(TxtLedgerName.Text) <> UCase(SupplierNameToEdit) Then
                If UCase(Replace(TxtLedgerName.Text, " ", "")) <> UCase(Replace(SupplierNameToEdit, " ", "")) Then
                    If SQLIsFieldExists("SELECT TOP 1 1 from   ledgers where LedgerNameTemp=N'" & Replace(TxtLedgerName.Text, " ", "") & "'") = True Then
                        MsgBox("The Supplier Account with the same name is Exists, Please Try again...")
                        TxtLedgerName.Focus()
                        Exit Sub
                    End If
                End If
        ...
        End If
        If IsOpenToAlter = True Then
            OpenedCurrencyRate = SQLGetNumericFieldValue("select ConRate from ledgerbook WHERE LEDGERNAME=N'" & SupplierNameToEdit & "' AND TRANSCODE=0", "ConRate")
            If OpenedCurrencyRate = 0 Then OpenedCurrencyRate = 1
            ExecuteSQLQuery("DELETE FROM LEDGERS WHERE LEDGERNAME=N'" & SupplierNameToEdit & "'")
            ExecuteSQLQuery("DELETE FROM Bill2bill WHERE LEDGERNAME=N'" & SupplierNameToEdit & "' and isopening=1")
            ExecuteSQLQuery("DELETE FROM LEDGERBOOK WHERE LEDGERNAME=N'" & SupplierNameToEdit & "' AND TRANSCODE=0")
        Else
            TxtCode.Text = GetAndSetIDCode(EnumIDType.SupplierID)
        ...
        SqlStr = "INSERT INTO [ledgers] ([LedgerName],[LedgerCode],[TaxRegno],[AccountGroup],[address],[location],[state],[country],[Tel],[fax],[emailid],[website]," _
            ...
            & "[pemail],[Dr],[Cr],[OpDr],[OpCr],[IsBillWise],[photopath],[currentbalance],[partyaddresscity],[StoreName],[CreditPeriod],[Currency],[LedgerNameTemp],[Activity],[pincode],[IsAllowCostCentre]) " _
            ...
            & "@photopath,@currentbalance,@partyaddresscity,@StoreName,@CreditPeriod,@Currency,@LedgerNameTemp,@Activity,@pincode,@IsAllowCostCentre)"
        ...
        If isImagechanged = True Then
            ...
            Else
                ...
                End If
            End If
        ElseIf IsOpenToAlter = False Then
            If Photoname.Length > 0 Then
                UpdateImageIntoDatabase(Photoname, "imagedata", "UPDATE IMAGES SET IMAGEDATA=@IMAGEDATA WHERE BCODE=N'LEDGER" & TxtCode.Text & "'", "")
            Else
                UpdateImageIntoDatabase(TxtPic.Image, "imagedata", "UPDATE IMAGES SET IMAGEDATA=@IMAGEDATA WHERE BCODE=N'LEDGER" & TxtCode.Text & "'", "")
        ...
        End If
        If IsOpenToAlter = True Then
            UpdateLogFile(DefStoreName, 0, "Alter Supplier", 0, CurrentUserName, "Alterd", System.Environment.MachineName, "Supplier was edited")
            AlterLedgerName(TxtLedgerName.Text, SupplierNameToEdit)
            ...
            End If
        Else
            UpdateLogFile(DefStoreName, 0, "New Supplier", 0, CurrentUserName, "Created", System.Environment.MachineName, "New Supplier is created")
            NewCreatedLedgerName = TxtLedgerName.Text
            loadDefaults()
            TxtLedgerName.Focus()
    ...
    Private Sub BtnCostCentre_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnCostCentre.Click
        ...
        End If
        Dim lfrm As New Costcenterallocation(CostList, AmValue, SupplierNameToEdit)
        lfrm.ShowDialog()
    End Sub
    ...
    Sub SaveCostCentersData(ByVal trancode As Double)
        ExecuteSQLQuery("delete from costcenterbook where TransCode=0 and ledgername=N'" & SupplierNameToEdit & "'")
    ...
    Sub loadCostCenterData()
        ...
        Try
            ...
            SQLFcmd.CommandText = "select * from CostCenterBook where IsAdditional='False' and transcode=0 and ledgername=N'" & SupplierNameToEdit & "' order by sno"
    ...
    Private Sub TxtSeletedLedgerName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles TxtSeletedLedgerName.SelectedIndexChanged
        SupplierNameToEdit = TxtSeletedLedgerName.Text
