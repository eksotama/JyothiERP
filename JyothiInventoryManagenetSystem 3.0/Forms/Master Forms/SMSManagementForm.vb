Public Class SMSManagementForm
    ...
    Private Sub TxtList_CellValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellValidatingEventArgs) Handles TxtList.CellValidating
        ...
        If e.ColumnIndex = TxtList.Columns("Mobile No").Index Then
            If e.FormattedValue.ToString.Length < 20 Then
                ExecuteSQLQuery("UPDATE LEDGERS SET tel=N'" & e.FormattedValue & "' where ledgername=N'" & TxtList.Item("Customer Name", e.RowIndex).Value & "'")
                e.Cancel = False
            Else
                e.Cancel = True
            End If
        ElseIf e.ColumnIndex = TxtList.Columns("Is Send SMS").Index Then
            If e.FormattedValue = True Then
                ExecuteSQLQuery("UPDATE LEDGERS SET IsSendSMS='True' where ledgername=N'" & TxtList.Item("Customer Name", e.RowIndex).Value & "'")
                e.Cancel = False
            ElseIf e.FormattedValue = False Then
                ExecuteSQLQuery("UPDATE LEDGERS SET IsSendSMS='False' where ledgername=N'" & TxtList.Item("Customer Name", e.RowIndex).Value & "'")
                e.Cancel = False
            Else
                e.Cancel = True
            End If
...
#Region "Functions"
    ...
    Sub LoadCustomerList(Optional ByVal sqlText As String = "")
        ...
        Sqlstr = "SELECT ROW_NUMBER() OVER (ORDER BY LedgerCode) AS [SNo], LedgerCode as [Code], LedgerName as [Customer Name],Tel as [Mobile No], IsSendSMS as [Is Send SMS] from ledgers where (Accountgroup in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.CustomersAccounts & "'))"
...
#End Region
    ...
    Private Sub SMSManagementForm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub ImsButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImsButton4.Click, PrintToolStripMenuItem.Click
        ...
        Me.Cursor = Cursors.WaitCursor
        Dim ds As New CustomerMobileReportDataSet
        ds.Clear()
        ...
        Dim objRpt As New CustomerMobileCRReport
        ...
        If PrintOptionsforCR.IsPrintHeader = False Then
            ...
            CType(objRpt.Section1.ReportObjects.Item("txttitle"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "CUSTOMERS CONTACT NUMBERS REPORTS"
        ...
        Else
            CType(objRpt.Section1.ReportObjects.Item("TxtPeriod"), CrystalDecisions.CrystalReports.Engine.TextObject).ObjectFormat.EnableCanGrow = True
            CType(objRpt.Section1.ReportObjects.Item("TxtPeriod"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = "CUSTOMERS CONTACT NUMBERS REPORTS"
