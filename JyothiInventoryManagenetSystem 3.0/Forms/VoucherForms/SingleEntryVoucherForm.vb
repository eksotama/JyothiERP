...
Public Class SingleEntryVoucherForm
    ...
    Sub New(ByVal VoucherNameValue As String, Optional openedTranscode As Double = 0)
        ...
        ElseIf VoucherNameValue = "Contra" Then
            ...
            PostedBox.Visible = True
            BtnPosted.Visible = True
            LoadDataIntoComboBox("select ledgername from Ledgers where Isactive=1 and Accountgroup  in (select subgroup from AccountGroupsList where groupname=N'" & AccountGroupNames.BankAccounts & "' or groupname=N'" & AccountGroupNames.BankOD & "' or groupname=N'" & AccountGroupNames.CashAccounts & "')", TxtPayLedgerName, "ledgername")
        Else
            MsgBox("Not Found")
        End If
    ...
    Public Sub ASaveBeforeCheck()
        ...
        If CDbl(txtCreditTotal.Text) = 0 Then
            MsgBox("There are no data to Save ....", MsgBoxStyle.Information)
            Exit Sub
        End If
    ...
    Sub SaveVoucher2()
        ...
        If vhtype = VoucherType.Payment Then
            CreditName = TxtPayLedgerName.Text
            DebitName = PayList.GetItems(1, 0)

        ElseIf vhtype = VoucherType.Receipt Then
            DebitName = TxtPayLedgerName.Text
            CreditName = PayList.GetItems(1, 0)
        ElseIf vhtype = VoucherType.Contra Then
            DebitName = TxtPayLedgerName.Text
            CreditName = PayList.GetItems(1, 0)
        End If
        ...
        Try
            ...
            With DBFR.Parameters
                ...
                If vhtype = VoucherType.Payment Then
                    .AddWithValue("Dr", 0)
                    .AddWithValue("Cr", CDbl(txtCreditTotal.Text))
                    .AddWithValue("AcLedger", DebitName)
                ElseIf vhtype = VoucherType.Receipt Then
                    .AddWithValue("Dr", CDbl(txtCreditTotal.Text))
                    .AddWithValue("Cr", 0)
        ...
        'dbf.Open("select * from ledgerbook", Conn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockOptimistic)
        For i As Integer = 0 To PayList.Rowcount - 1
            If Len(PayList.GetItems(1, i)) > 2 Then
                ...
                With DBFR.Parameters
                    ...
                    sno = sno + 1
                    If vhtype = VoucherType.Payment Then
                        If CDbl(PayList.GetItems(2, i)) > 0 Then
                            .AddWithValue("Dr", CDbl(PayList.GetItems(2, i)))
                            .AddWithValue("Cr", 0)
                            .AddWithValue("AcLedger", CreditName)
                        Else
                            .AddWithValue("Dr", 0)
                            .AddWithValue("Cr", -1 * CDbl(PayList.GetItems(2, i)))
                            .AddWithValue("AcLedger", DebitName)
                        End If
                    ElseIf vhtype = VoucherType.Receipt Or vhtype = VoucherType.Contra Then
                        If CDbl(PayList.GetItems(2, i)) > 0 Then
                            .AddWithValue("Dr", 0)
                            .AddWithValue("Cr", CDbl(PayList.GetItems(2, i)))
                            .AddWithValue("AcLedger", DebitName)
                        Else
                            .AddWithValue("Dr", -1 * CDbl(PayList.GetItems(2, i)))
                            .AddWithValue("Cr", 0)
                            .AddWithValue("AcLedger", CreditName)
                        End If
                    ...
                    If vhname.Length > 0 Then
                        If MsgBox("Do you want to Send SMS to Customer Mobile ?       ", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                            ...
                            If mbno.Length > 6 Then
                                ...
                                TempStr = TempStr.Replace("@PAYMENTBY@", TxtPayLedgerName.Text)
    ...
    Sub SaveCostCentersData(ByVal trancode As Double)
        ...
        For i As Integer = 0 To PayList.Rowcount - 1
            If Len(PayList.GetItems(1, i)) > 2 Then
                If CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.RowCount > 0 Then
                    For rc As Integer = 0 To CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.RowCount - 1
                        If Len(CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostname", rc).Value) > 0 Then
                            Dim SqlStr As String = ""
                            ...
                            With DBFR.Parameters
                                .AddWithValue("sno", Sno)
                                .AddWithValue("LedgerName", PayList.GetItems(1, i).ToString)
                                .AddWithValue("CostCenterName", CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostname", rc).Value)
                                If PayList.GetItems(0, i) = "Cr" Then
                                    .AddWithValue("Dr", 0)
                                    .AddWithValue("Cr", CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tamount", rc).Value)
                                Else
                                    .AddWithValue("Dr", CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tamount", rc).Value)
                                    .AddWithValue("Cr", 0)
                                End If
                                .AddWithValue("UserName", CurrentUserName)
                                .AddWithValue("TransCode", trancode)
                                ...
                                .AddWithValue("VoucherName", VoucherName)
                                .AddWithValue("InvoiceNo", TxtVoucherNo2.Text)
                                .AddWithValue("CostCat", CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostcat", rc).Value)
                                .AddWithValue("CostNo", CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostno", rc).Value)
    ...
    Public Sub OpenVoucherByID(ByVal openedID As Single, Optional ByVal Mode As Integer = 0)
                ...
                While Sreader.Read()
                    ...
                    Else
                        ...
                        If i > 1 Then
                            PayList.AddRow()
                        End If
                        PayList.SetItem(1, i - 1) = Sreader("LedgerName").ToString.Trim
                        If CDbl(Sreader("cr")) <> 0 Then
                            PayList.SetItem(0, i - 1) = "Cr"
                            PayList.SetItem(2, i - 1) = Sreader("cr")
                        Else
                            PayList.SetItem(0, i - 1) = "Dr"
                            PayList.SetItem(2, i - 1) = Sreader("Dr")
                        End If
                        PayList.SetItem(3, i - 1) = Sreader("conrate")
                        i = i + 1
                    End If
                End While
        ...
        For i As Integer = 0 To PayList.Rowcount - 1
            ...
            Dim rno As Integer = 0
            CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Rows.Clear()
            Try
                SqlConn.ConnectionString = ConnectionStrinG
                SqlConn.Open()
                SQLFcmd.Connection = SqlConn
                ...
                Sreader = SQLFcmd.ExecuteReader
                While Sreader.Read()
                    rno = CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Rows.Add
                    CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostname", rno).Value = Sreader("CostCenterName").ToString.Trim
                    CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tamount", rno).Value = Sreader("dr") + Sreader("cr")
                    CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostcat", rno).Value = Sreader("costcat").ToString.Trim
                    CType(PayList.Tb.Controls(i), SingleEntryVoucherRow).CostList.Item("tcostno", rno).Value = Sreader("costno").ToString.Trim
    ...
    Private Sub TransList_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TransList.KeyDown
    ...
    Private Sub VoucherEntryControl1_AmountEnteredEvent(ByVal e As Object) Handles PayList.AmountEnteredEvent
        txtCreditTotal.Text = FormatNumber(PayList.GetCreditTotal, ErpDecimalPlaces)
        '   txtDebitTotal.Text = FormatNumber(PayList.GetDebitTotal)
    End Sub
    ...
    Private Sub TxtPayLedgerName_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtPayLedgerName.LostFocus
        ConRate = GetCurrencyRate(SQLGetStringFieldValue("SELECT CURRENCY FROM LEDGERS WHERE LEDGERNAME=N'" & TxtPayLedgerName.Text & "'", "CURRENCY"))
        TxtCurrentbalance.Text = "Current " & GetCurrentBalanceText(TxtPayLedgerName.Text)
    End Sub
    ...
    Private Sub PayList_Load(sender As System.Object, e As System.EventArgs) Handles PayList.Load
            ...
            Try
                ...
                While Sreader.Read()
                    ...
                    If IsBankLedger(QcreditName) = True Then
                        ...
                        Prtchequevalues.PayeeName = QDebitName
    ...
    Private Sub Delete_Click(sender As System.Object, e As System.EventArgs) Handles BtnDelete.Click, DeleteToolStripMenuItem.Click
        ...
        If MsgBox("Do You want to Delete the Voucher Details =" & TransList.Item(2, TransList.CurrentRow.Index).Value & " and Voucher No =" & TransList.Item(3, TransList.CurrentRow.Index).Value & " ?", MsgBoxStyle.YesNo + MsgBoxStyle.Critical + MsgBoxStyle.DefaultButton2, "Delete The Voucher ?") = MsgBoxResult.Yes Then
            '   Dim k As String
            '   k = "Voucher No =" & TransList.Item(3, TransList.CurrentRow.Index).Value
            '   ExecuteSQLQuery("delete  from ledgerbook where transcode=" & TransList.Item(0, TransList.CurrentRow.Index).Value)
            ...
            LoadDef()
            ' SetStatus("Deleted Successfully " & k)
        End If
    End Sub
