...
Public Class Materialtransferrequestfrm
    ...
    Private Sub TxtStockCode_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtStockCode.KeyPress, TxtStockSize.KeyPress, TxtStockName.KeyPress
        If ValidateLocations() = False Then
            MsgBox("Please select the locations......")
            Exit Sub
        End If
    ...
    Sub GetStockDets(ByVal Ctrlno As Byte, ByVal curchar As String)
        ...
        Dim k As New ChooseItems(bvalue, TxtLocationFrom.Text)
    ...
    Private Sub BtmAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtmAdd.Click
        ...
        ElseIf MyAppSettings.IsAllowNegativeStock = False And IsInvoiceOpen = True Then
            If TxtStockQty.GetTotalQty() - SQLGetNumericFieldValue("select totalqty from StockTransferRowItems where barcode=N'" & NewAddItem.StockItemBarCode & "'", "totalqty") > SQLGetNumericFieldValue("select totalqty from stockdbf where barcode=N'" & NewAddItem.StockItemBarCode & "'", "totalqty") Then
                MsgBox("The Stock Quantity is greater than the available stock....", MsgBoxStyle.Information)
                TxtStockQty.Focus()
                Exit Sub
            End If
    ...
    Sub CheckBeforeSave()
        ...
        If ValidateLocations() = False Then
            MsgBox("Please change locations......")
            Exit Sub
        End If
    ...
    Sub SaveData()
        ...
        If IsInvoiceOpen = True Then
            ...
            ExecuteSQLQuery("delete from StockTransferDetails where transcode=" & OpenedTranscode)
            ExecuteSQLQuery("delete from StockTransferRowItems where transcode=" & OpenedTranscode)
    ...
    Sub SaveStockItemsRows()
        ...
        Try
            SqlStr = "INSERT INTO [StockTransferDetails] ([TransCode],[TransDate],[TransDateValue],[QutoNo],[QutoRef],[OrderNo],[OrderRef],[DNoteno]," _
        ...
        SqlStr = "INSERT INTO [StockTransferRowItems]([TransCode],[TransDate],[TransDateValue],[QutoNo],[QutoRef],[OrderNo],[OrderRef]," _
        ...
        For i As Integer = 0 To TxtFromList.RowCount - 1
            If TxtFromList.Item("SfBarCode", i).Value.ToString.Length > 0 Then
                ...
                'PresentStockAtFromLocation = SQLGetNumericFieldValue("select TotalQty from stockdbf where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "'", "TotalQty")
                ...
                'ExecuteSQLQuery("update stockdbf set TotalQty=" & TempQtyFromBox.GetTotalQty & ",BaseQty=" & TempQtyFromBox.GetUnitQtys(0) & ",SubUnitQty=" & TempQtyFromBox.GetUnitQtys(1) & ",QtyText=N'" & TempQtyFromBox.GetTotalQtyText() & "' where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "'")
                'UpdateStockItemPriceMethod(st.StockItemCode, st.StockITemSize, "", st.StockLocationNames)

                '' UPDATE BATCHNO QTY
                'TempQtyFromBox.AllowNegetiveValues = True
                'PresentStockAtFromLocation = SQLGetNumericFieldValue("select TotalQty from STOCKBATCH where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "' AND batchno=N'" & TxtFromList.Item("sfbatchno", i).Value & "'", "TotalQty")
                'TempQtyFromBox.SetUnitName(st.TxtQtyBOX.GetUnitName)
                'TempQtyFromBox.TxtQty2.Text = "0"
                'TempQtyFromBox.TxtQty1.Text = "0"
                'TempQtyFromBox.AddStockValue(PresentStockAtFromLocation - st.TxtQtyBOX.GetTotalQty)
                'ExecuteSQLQuery("update STOCKBATCH set TotalQty=" & TempQtyFromBox.GetTotalQty & ",BaseQty=" & TempQtyFromBox.GetUnitQtys(0) & ",SubUnitQty=" & TempQtyFromBox.GetUnitQtys(1) & ",QtyText=N'" & TempQtyFromBox.GetTotalQtyText() & "' where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "' AND batchno=N'" & TxtFromList.Item("sfbatchno", i).Value & "'")
        ...
        For i As Integer = 0 To TxtToList.RowCount - 1
            ...
            If TxtToList.Item("STBarCode", i).Value.ToString.Length > 0 Then
                ...
                'UPDATE BATCHNO QTY
                'IF BATCHNO ROW IS NOT EXISTS,THEN CREATE A ROW FOR A PARTICULAR LOCATION
                If Len(TxtToList.Item("sTbatchno", i).Value) > 0 Then
                    If SQLIsFieldExists("select TotalQty from STOCKBATCH where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "'  AND batchno=N'" & TxtToList.Item("sTbatchno", i).Value & "'") = False Then
                        MAINCON.ConnectionString = ConnectionStrinG
                        MAINCON.Open()
                        ...
                        Dim DBF3 As New SqlClient.SqlCommand(batchsqlstr, MAINCON)
                        With DBF3.Parameters
                            .AddWithValue("StockCode", st.StockItemCode)
                            .AddWithValue("Barcode", st.StockItemBarCode)
                            .AddWithValue("StockSize", st.StockITemSize)
                            .AddWithValue("Location", st.StockLocation)
                            ...
                            DT.Value = CDate(TxtToList.Item("stExpiry", i).Value)
                ...
                'TempQtyFromBox.AllowNegetiveValues = True
                'PresentStockAtFromLocation = SQLGetNumericFieldValue("select TotalQty from STOCKBATCH where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "'  AND batchno=N'" & TxtToList.Item("sTbatchno", i).Value & "'", "TotalQty")
                'TempQtyFromBox.SetUnitName(st.TxtQtyBOX.GetUnitName)
                'TempQtyFromBox.TxtQty2.Text = "0"
                'TempQtyFromBox.TxtQty1.Text = "0"
                ''adding current and new qty
                'TempQtyFromBox.AddStockValue(PresentStockAtFromLocation + st.TxtQtyBOX.GetTotalQty)
                'ExecuteSQLQuery("update STOCKBATCH set TotalQty=" & TempQtyFromBox.GetTotalQty & ",BaseQty=" & TempQtyFromBox.GetUnitQtys(0) & ",SubUnitQty=" & TempQtyFromBox.GetUnitQtys(1) & ",QtyText=N'" & TempQtyFromBox.GetTotalQtyText() & "' where stockcode=N'" & st.StockItemCode & "' and stocksize=N'" & st.StockITemSize & "' and location=N'" & st.StockLocationNames & "'  AND batchno=N'" & TxtToList.Item("sTbatchno", i).Value & "'")
    ...
    Private Sub ImsButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImsButton4.Click, PrintToolStripMenuItem.Click
        ...
        If MyAppSettings.IsShowAdditionalFieldsinSalesDeliveryNote = True Then
            ...
            cnn.Open()
            Dim dscmd As New SqlDataAdapter("SELECT * FROM StockTransferRowItems as StockInvoiceRowItems WHERE vouchername='SJout' and TRANSCODE=" & OpenedTranscode, cnn)
            ...
            CType(objRpt.Section2.ReportObjects.Item("txtfromlocation"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = TxtLocationFrom.Text
            CType(objRpt.Section2.ReportObjects.Item("txttolocation"), CrystalDecisions.CrystalReports.Engine.TextObject).Text = TxtLocationTo.Text
    ...
    End Sub
    Function ValidateLocations() As Boolean
        Dim k As Boolean = True
        If TxtLocationFrom.Text.Length = 0 Then
            Return False
        End If
        If TxtLocationTo.Text.Length = 0 Then
            Return False
        End If
        If TxtLocationTo.Text = TxtLocationFrom.Text Then
            Return False
        End If

        Return k
    End Function
    Sub CopyLocationDataIntoGrid()
        If IMSBEGINTRANSACTION() = False Then
            IMSENDTRANSACTION()
            Exit Sub
        End If
        TxtToList.Rows.Clear()
        Dim newrowno As Integer = 0
        For rno As Integer = 0 To TxtFromList.RowCount - 1
            newrowno = TxtToList.Rows.Add
            For colno As Integer = 0 To TxtFromList.ColumnCount - 1
                TxtToList.Item(colno, newrowno).Value = TxtFromList.Item(colno, rno).Value
            Next
            Dim Bcode As String = ""
            If SQLIsFieldExists("select stockname from stockdbf where stockcode=N'" & TxtFromList.Item("sfitemcode", rno).Value & "' and stocksize=N'" & TxtFromList.Item("sfsize", rno).Value & "' and location=N'" & TxtLocationTo.Text & "'") = False Then
                Dim snewfrm As New CreateNewStockItem(TxtFromList.Item("sfitemcode", rno).Value, TxtFromList.Item("sfsize", rno).Value)
                Dim NewAddItem As New ChooseItemClass

                LoadStockItemsIntoClassWithStockDetails(TxtFromList.Item("sfitemcode", rno).Value, TxtFromList.Item("sfsize", rno).Value, "", NewAddItem)
                AddStockItem(TxtFromList.Item("sfitemcode", rno).Value, TxtFromList.Item("sfsize", rno).Value, "", TxtLocationTo.Text, NewAddItem)

            End If

            TxtToList.Item("stlocation", newrowno).Value = TxtLocationTo.Text
            Bcode = SQLGetStringFieldValue("select Barcode from stockdbf where stockcode=N'" & TxtFromList.Item("sfitemcode", rno).Value & "' and stocksize=N'" & TxtFromList.Item("sfsize", rno).Value & "' and location=N'" & TxtLocationTo.Text & "'", "Barcode")
            TxtToList.Item("stbarcode", newrowno).Value = Bcode
            Bcode = SQLGetStringFieldValue("select custBarcode from stockdbf where stockcode=N'" & TxtFromList.Item("sfitemcode", rno).Value & "' and stocksize=N'" & TxtFromList.Item("sfsize", rno).Value & "' and location=N'" & TxtLocationTo.Text & "'", "custBarcode")
            TxtToList.Item("stcustbarcode", newrowno).Value = Bcode
