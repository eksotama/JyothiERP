...
Public Class EmployeeMonthlyAbsentAttendsForm
    ...
    Sub lOADdATA()
        ...
        Sqlstr = "SELECT  UPPER(LEFT(DATENAME(MONTH, TransDate), 3)) + '-' + CONVERT(varchar(30), YEAR(TransDate)) AS Period,(select empid from employees where employees.empname=EmpAttend.empname) as [EMP ID], EmpName as [Employee Name], " _
            ...
            & ", 0 as [Total Absents],0 as [Total leaves] " _
        ...
        Try
            ...
            TxtList.Columns("Total Presents").Visible = False

            TxtList.Columns("Total Absents").AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            TxtList.Columns("Total Absents").Width = 80

            TxtList.Columns("Total leaves").AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            TxtList.Columns("Total leaves").Width = 80
        ...
        For i As Integer = 0 To TxtList.RowCount - 1
            ...
            Dim daysinmonths As Integer = 0
            Dim Monthname As String = ""
            Dim Year As Integer = 0
            Dim words As String() = TxtList.Item("Period", i).Value.ToString.Split("-")
            Monthname = words(0)
            Year = CInt(words(1))
            Dim monthNumber = DateTime.ParseExact(Monthname, "MMM", CultureInfo.CurrentCulture).Month
            daysinmonths = Date.DaysInMonth(Year, monthNumber)
            DATEFROMVALUE.Value = New Date(Year, monthNumber, 1)
            datetovalue.Value = New Date(Year, monthNumber, daysinmonths)
            Dim totabs As Double = 0
            Dim TotalLeaves As Double = 0
            TotalLeaves = GetTotalempALLPaideaves(TxtList.Item("Employee Name", i).Value, DATEFROMVALUE, datetovalue) + GetTotalCompanyLeaves(DATEFROMVALUE, datetovalue)
            totabs = daysinmonths - (TotalLeaves + CDbl(TxtList.Item("Total Presents", i).Value))
            TxtList.Item("Total Absents", i).Value = totabs
            TxtList.Item("Total leaves", i).Value = TotalLeaves
        Next
        TxtList.ResumeLayout()
        MainForm.Cursor = Cursors.Default
    End Sub

    Private Sub EmployeeMonthlyAbsentAttendsForm_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub ImsButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImsButton2.Click
        ...
        Me.Cursor = Cursors.WaitCursor
        Dim ds As New EmployeeMonthlyAttendDataSet
        ds.Clear()
        ...
        Dim objRpt As New EmpMonthlyAttendenceAbsentsCRReport
    ...
    Private Sub BtnPrintempwise_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnPrintempwise.Click
        ...
        Dim objRpt As New EmpMonthlyAttendenceCRReport___Empwise
