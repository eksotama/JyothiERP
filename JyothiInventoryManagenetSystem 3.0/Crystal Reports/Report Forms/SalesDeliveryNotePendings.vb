Public Class SalesDeliveryNotePendings
    ...
    Sub LoadListofOrders(Optional ByVal AddSqlText As String = "")
        If IsShowPendingsOnly.Checked = True Then
            SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [Order No],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Customer Name],nettotal as [Invoice Amount],(case IsPending when 0 then 'Completed' ELSE  'Pending' END)  as [Status] from stockinvoicedetails where Isdelete=0 and IsPending=1 and vouchername='SD' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") AND TRANSCODE IN ( SELECT TRANSCODE FROM STOCKINVOICEROWITEMS WHERE USEDQTY=0 AND VOUCHERNAME='SD') "
        Else
            SqlStr = "SELECT ROW_NUMBER() OVER (ORDER BY QutoNo) AS [S.NO],TransCode as [TransCode],TransDate as [Date],QutoNo as [Order No],QutoRef as [Ref no],DelivaryDate as [Delivary Date],LedgerName as [Customer Name],nettotal as [Invoice Amount],(case IsPending when 0 then 'Completed' ELSE  'Pending' END)  as [Status] from stockinvoicedetails where Isdelete=0 and vouchername='SD' and (Transdatevalue between " & (CDbl(TxtStartDate.Value.Date.ToOADate)) & " and " & (CDbl(TxtEndDate.Value.Date.ToOADate)) & ") AND TRANSCODE IN ( SELECT TRANSCODE FROM STOCKINVOICEROWITEMS WHERE USEDQTY=0 AND VOUCHERNAME='SD') "
        End If
    ...
    Private Sub SalesDeliveryNotePendings_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
