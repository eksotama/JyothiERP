...
Public Class VATSalesDailyReport
    ...
    Sub LoadReport()
        ...
        Try
            ...
            While Sreader.Read()
                ...
                Else
                    ...
                    VatValues.Items.Add("[" & Sreader("outputvatledger") & "]")
                End If
        ...
        Next
        If Subquery.Length > 2 Then
            SQLstr = "SELECT CONVERT(date,[date]) AS [DATE],(select top 1 qutono from stockinvoicedetails where convert(date,transdate)=CONVERT(date,[date]) and (VoucherName='SI' or voucherName='POS') order by transcode,transdate) +' To ' +(select top 1 qutono from stockinvoicedetails where convert(date,transdate)=CONVERT(date,[date]) and (VoucherName='SI' or voucherName='POS') order by transcode desc,transdate desc) as [Invoice Range] ,count(*) as [Total Invoices],Sum(Gross) as InvoiceTotal, " & Subquery & " FROM (" & SQLstr & ") SS GROUP BY CONVERT(date,[date]) ORDER BY [DATE]"
        Else
            SQLstr = "SELECT CONVERT(date,[date]) AS [DATE],(select top 1 qutono from stockinvoicedetails where convert(date,transdate)=CONVERT(date,[date]) and (VoucherName='SI' or voucherName='POS') order by transcode,transdate) +' To ' +(select top 1 qutono from stockinvoicedetails where convert(date,transdate)=CONVERT(date,[date]) and (VoucherName='SI' or voucherName='POS') order by transcode desc,transdate desc) as [Invoice Range],count(*) as [Total Invoices],Sum(Gross) as InvoiceTotal FROM (" & SQLstr & ") SS GROUP BY CONVERT(date,[date]) ORDER BY [DATE]"
        End If
        ...
        For j = ROWNO To dbtable.RowCount - 1
            ...
            For i As Integer = 0 To dbtable.ColumnCount - 1
                ...
                If dbtable.Columns(i).Visible = True Then
                    If dbtable.Columns(i).DefaultCellStyle.Alignment = DataGridViewContentAlignment.TopLeft Then
                        drawFormat.Alignment = StringAlignment.Near
                        Dim Rect2 As New Rectangle(Rect1.X + 2, Rect1.Y, dbtable.Columns(i).Width, 15)
