...
Public Class VATAllSalesVouchers
    ...
    Sub LoadReport()
        ...
        Try
            ...
            While Sreader.Read()
                If Sreader("VATTAX") = 0 Then
                    VatsalesSqlStr = VatsalesSqlStr & ",(SELECT SUM(DR+CR) AS TOT FROM LEDGERBOOK WHERE LEDGERNAME=N'" & Sreader("SalesLedger") & "' AND transcode=StockInvoiceDetails.transcode) as [" & Sreader("SalesLedger") & "]"
                Else
                    VatsalesSqlStr = VatsalesSqlStr & ",(SELECT SUM(DR+CR) AS TOT FROM LEDGERBOOK WHERE LEDGERNAME=N'" & Sreader("SalesLedger") & "' AND transcode=StockInvoiceDetails.transcode) as [" & Sreader("SalesLedger") & "]"
                    VATSqlStr = VATSqlStr & ",(SELECT SUM(DR+CR) AS TOT FROM LEDGERBOOK WHERE LEDGERNAME=N'" & Sreader("outputvatledger") & "' AND transcode=StockInvoiceDetails.transcode) as [" & Sreader("outputvatledger") & "]"
                End If
        ...
        SqlStrTotal = "SELECT SUM(nettotal) AS Tot from StockInvoiceDetails where (VoucherName='SI' or voucherName='POS')  and isdelete=0 " & Vhstr
        SQLstr = "select  QutoNo as [Invoice No],TransDate as [Date], LedgerName as [Particulars],transtype as [Voucher Type],(select taxregno from ledgers where ledgers.ledgername=StockInvoiceDetails.ledgername) as [Sales Tax No/TIN Number] , grosstotal as [Gross]" & VatsalesSqlStr & VATSqlStr & " ,TransCode as [Trans Code] from StockInvoiceDetails where (VoucherName='SI' or voucherName='POS') and isdelete=0  " & Vhstr
        sumcolno = 4
        GraphSqlStr = "(VoucherName='SI' or voucherName='POS') and isdelete=0  " & Vhstr
