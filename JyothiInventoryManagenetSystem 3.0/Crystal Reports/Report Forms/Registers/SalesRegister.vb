...
'SELECT     (CASE subunitqty WHEN 0 THEN FLOOR(TotalQty / UnitCon) ELSE CAST(TotalQty AS INT) % CAST(UnitCon AS INT) END) AS jvsk
'FROM         dbo.StockInvoiceRowItems
Public Class SalesRegister
    ...
    Sub LoadReport()
        ...
        If IsLedgerCurrency.Checked = True Then
            SqlStr = "select  QutoNo as [Invoice No],QutoRef as [Ref No],TransDate as [Date], LedgerName as [Customer Name],SalesPerson as [Sales Person] , grosstotal*CurrencyCon1 as [Gross],Additions*CurrencyCon1 as [Additions],Deductions*CurrencyCon1 as [Deductions], (Currency + ' ' + CONVERT(varchar(30), CAST (nettotal*CurrencyCon1 AS DECIMAL(15,3)),1))  as [Net Total],TransCode as [Trans Code] from StockInvoiceDetails where (VoucherName='SI' or voucherName='POS') and isdelete=0 "
        Else
            SqlStr = "select  QutoNo as [Invoice No],QutoRef as [Ref No],TransDate as [Date], LedgerName as [Customer Name],SalesPerson as [Sales Person] , grosstotal as [Gross],Additions as [Additions],Deductions as [Deductions], CONVERT(varchar(30), CAST (nettotal AS DECIMAL(15,3)),1)  as [Net Total],TransCode as [Trans Code] from StockInvoiceDetails where (VoucherName='SI' or voucherName='POS') and isdelete=0  "
        ...
        End If
        If TxtBySaleForm.Text.Length = 0 Or TxtBySaleForm.Text = "*All" Then
        Else
            SqlStr = SqlStr & " and transtype=N'" & TxtBySaleForm.Text & "' "
            SqlStrTotal = SqlStrTotal & " and transtype=N'" & TxtBySaleForm.Text & "' "
            GraphSqlStr = GraphSqlStr & " and transtype=N'" & TxtBySaleForm.Text & "' "
    ...
    Sub LoadItemWiseReport()
        ...
        Else
            ...
            SqlGroupStr = SqlGroupStr & " and transtype=N'" & TxtBySaleForm.Text & "' "
        End If
    ...
    Private Sub TxtSalesPerson_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtSalesPerson.SelectedIndexChanged, TxtBySaleForm.SelectedIndexChanged
        If TxtShowItemDetails.Checked = True Then
            LoadItemWiseReport()
        Else
            LoadReport()
    ...
    Private Sub SalesRegister_Activated(sender As Object, e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub SalesRegister_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        LoadDataIntoComboBox("select distinct transtype from StockInvoiceDetails", TxtBySaleForm, "transtype", "*All")
